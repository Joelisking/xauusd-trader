...
public:
    ...
    // ------------------------------------------------------------------
    // Swing-specific: check if news event is within N hours
    // ------------------------------------------------------------------

    bool IsEventWithinHours(int hours)
    {
        if(m_next_event_time == 0 || m_next_event_impact < 2)
            return false;
        ...
        // Also check if we just passed an event recently (within post window)
        int seconds_after = (int)(now - m_next_event_time);

        return (seconds_to_event >= 0 && seconds_to_event <= hours * 3600) ||
               (seconds_after >= 0 && seconds_after <= m_during_seconds);
    }

    // Check if event is specifically NFP, CPI, or FOMC (major events)
    bool IsMajorEventWithinHours(int hours)
    {
        if(!IsEventWithinHours(hours))
            return false;

        // High impact = 3
        return (m_next_event_impact >= 3);
    }
};
